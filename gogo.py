...
def search(keyword: str) -> list[BeautifulSoup]:
    ...
    response = requests.get(search, timeout=60).content
    soup = BeautifulSoup(response, parser)
    results_page = soup.find('ul', class_="items")
    results = results_page.find_all('li')
    return results

def extract_anime_title_and_page_link(result: BeautifulSoup) -> tuple[str | None]:
...
def get_download_page(episode_page_link: str) -> str:
    response = requests.get(episode_page_link, timeout=60).content
    soup = BeautifulSoup(response, parser)
...
class Download():
    ...
    def download(self):
        response = requests.get(self.link, stream=True, timeout=60)
...
def extract_poster_summary_and_episode_count(anime_page_link: str) -> tuple[str]:
    response = requests.get(anime_page_link, timeout=60).content
    soup = BeautifulSoup(response, parser)
    ...
    episode_count = soup.find('ul', id='episode_page').find_all('li')[-1].find('a').get_text().split('-')[-1]
    return (poster, summary, int(episode_count))
...
def main():
    # Download settings
    default_download_folder = r'E:\Anime'
    results = search(input('Enter the anime you would like to download: '))
    start_episode = int(input("What episode would you like to start downloading from: "))
    end_episode = int(input("What episode would you like to stop downloading at: "))
    quality = '360'
    ...
    _, _, count = extract_poster_summary_and_episode_count(anime_page_link)
    print(count)
    dub_link = dub_available(anime_title)
    print(dub_link)
